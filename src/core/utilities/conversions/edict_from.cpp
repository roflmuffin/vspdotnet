...
bool EdictFromIndex(int iEntityIndex, edict_t*& output) {
  if (iEntityIndex < 0 || iEntityIndex >= (int)globals::gpGlobals->maxEntities) return false;

  edict_t* pEdict;
#if defined(ENGINE_ORANGEBOX) || defined(ENGINE_BMS) || defined(ENGINE_GMOD)
  pEdict = engine->PEntityOfEntIndex(iEntityIndex);
#else
  pEdict = (edict_t*)(globals::gpGlobals->pEdicts + iEntityIndex);
#endif
  if (!pEdict || pEdict->IsFree() || !pEdict->GetUnknown()) return false;
...
bool EdictFromUserid(int iUserID, edict_t*& output) {
  for (int iCurrentIndex = 1; iCurrentIndex <= globals::gpGlobals->maxClients;
       iCurrentIndex++) {
    edict_t* pEdict;
    if (!EdictFromIndex(iCurrentIndex, pEdict)) continue;

    if (globals::engine->GetPlayerUserId(pEdict) == iUserID) {
      output = pEdict;
      return true;
    }
  }
...
bool EdictFromIntHandle(int iEntityHandle, edict_t*& output) {
  int iIndex;
  if (!IndexFromIntHandle(iEntityHandle, iIndex)) return false;
