...
bool AddressFromPlayerInfo2(IPlayerInfo* pInfo, const char*& output) {
  ...
  int index;
  if (!IndexFromPlayerInfo(pInfo, index)) return false;

  INetChannelInfo* netinfo = globals::engine->GetPlayerNetInfo(index);
...
bool AddressFromPlayerInfo(IPlayerInfo* pInfo, const char*& output) {
  ...
  if (!AddressFromPlayerInfo2(pInfo, result)) return false;
